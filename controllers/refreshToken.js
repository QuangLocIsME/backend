...
import { clearAuthCookies, generateAccessToken, setAccessTokenCookie } from '../utils/tokenUtils.js';
...
const refreshToken = async (req, res) => {
  try {
    ...
    const refreshTokenDoc =
      await RefreshToken.findOne({ token: refreshToken, isRevoked: false });
    if (!refreshTokenDoc) {
    ...
    jwt.verify(
      refreshToken, process.env.JWT_REFRESH_SECRET, async (err, decoded) => {
        if (err || !decoded.id ||
          decoded.id.toString() !== refreshTokenDoc.userId.toString()) {
          clearAuthCookies(res);
          return res.status(403).json(
    ...
    return res.status(500).json(
      { success: false, message: 'Lỗi server khi làm mới token' });
  }
};
